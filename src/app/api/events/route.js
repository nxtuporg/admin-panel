...
export async function POST(req) {
    ...
    if (!eventData.name || !eventData.day || !eventData.time || !eventData.address || !eventData.description) { 
...
export async function PUT(req) {
    ...
    try {
        const eventData = await req.json();
        const { _id } = eventData;
        const updatedEvent = await eventsModel.findByIdAndUpdate(_id, eventData, { new: true });
        if (!updatedEvent) {
            return NextResponse.json({ message: "Event not found", status: false }, { status: 404 });
        }
        return NextResponse.json({ message: "Event updated", status: true, event: updatedEvent }, { status: 200 });
    } catch (error) {
        return NextResponse.json({ message: error.message, status: false }, { status: 400 });
    }
}
...
export async function DELETE(req) {
    ...
    try {
        ...
        return NextResponse.json({ message: "Event deleted", status: true }, { status: 200 });
    } catch (error) {
        return NextResponse.json({ message: error.message, status: false }, { status: 400 });
    }
}
